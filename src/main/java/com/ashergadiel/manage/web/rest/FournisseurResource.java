...
/**
 * REST controller for managing {@link com.ashergadiel.manage.domain.Fournisseur}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class FournisseurResource {
        ...
        Optional<Fournisseur> result = fournisseurRepository
            ...
            .map(existingFournisseur -> {
                if (fournisseur.getCodeFournisseur() != null) {
                    existingFournisseur.setCodeFournisseur(fournisseur.getCodeFournisseur());
                }
                if (fournisseur.getRaisonFournisseur() != null) {
                    existingFournisseur.setRaisonFournisseur(fournisseur.getRaisonFournisseur());
                }
                if (fournisseur.getAdrfournisseur() != null) {
                    existingFournisseur.setAdrfournisseur(fournisseur.getAdrfournisseur());
                }
                if (fournisseur.getPaysFournisseur() != null) {
                    existingFournisseur.setPaysFournisseur(fournisseur.getPaysFournisseur());
                }
                if (fournisseur.getVilleFournisseur() != null) {
                    existingFournisseur.setVilleFournisseur(fournisseur.getVilleFournisseur());
                }
                if (fournisseur.getMailFournisseur() != null) {
                    existingFournisseur.setMailFournisseur(fournisseur.getMailFournisseur());
                }
                if (fournisseur.getCelFournisseur() != null) {
                    existingFournisseur.setCelFournisseur(fournisseur.getCelFournisseur());
                }
                if (fournisseur.getTelFournisseur() != null) {
                    existingFournisseur.setTelFournisseur(fournisseur.getTelFournisseur());
                }
    ...
    public List<Fournisseur> getAllFournisseurs(@RequestParam(required = false) String filter) {
        if ("documentachat-is-null".equals(filter)) {
            log.debug("REST request to get all Fournisseurs where documentAchat is null");
            return StreamSupport
                .stream(fournisseurRepository.findAll().spliterator(), false)
                .filter(fournisseur -> fournisseur.getDocumentAchat() == null)
